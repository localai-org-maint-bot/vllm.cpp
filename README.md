...
- **2026-08** **MiniMax-Music3 generates music end to end.** Run the standalone example or a
  music-only server on `POST /v1/audio/speech`; both use the versioned C ABI.
...
llama.cpp-style C ABI (`VLLM_ABI_VERSION 20`, 45 exported functions) suitable for `dlopen` / FFI.
