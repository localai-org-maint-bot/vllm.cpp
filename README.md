...
- **2026-08** **Model IDs work without a manual download.** Pass `org/repo` or `org/repo:TAG` to
  `--model`; the server downloads missing Hugging Face files into its cache and supports gated
  repositories through `HF_TOKEN`.
...
llama.cpp-style C ABI (`VLLM_ABI_VERSION 23`) suitable for `dlopen` / FFI.
